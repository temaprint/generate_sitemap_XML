...
def generate_sitemap(domain, url_list):
    urlset = ET.Element('urlset', xmlns="http://www.sitemaps.org/schemas/sitemap/0.9")

    for url in url_list:
        url_element = ET.SubElement(urlset, 'url')
        ET.SubElement(url_element, 'loc').text = f"{domain}/{url}"
...
def main():
    domain = 'https://icons8.com/lunacy'
    urls = []
    ...
    sitemap_xml = generate_sitemap(domain, urls)

    # Save gzipped XML
    save_gzipped_xml('domain.xml.gz', sitemap_xml)

    print('Sitemap generated and saved as domain.xml.gz!')
